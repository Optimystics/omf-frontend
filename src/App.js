...
const contractAddress ="0x6b11FC2cec86edeEd1F3705880deB9010F0D584B";
  ...
  const loadBlockchainData = async () => {
    ...
    try {
      ...
      if (window.ethereum) {
          ...
          // Switch to desired chain (e.g., chainId "0x64")
          try {
              ...
              if (switchError.code === 4902) {
                  // Handle the case where the desired network is not added to MetaMask
                  swal_error("The desired network is not added to MetaMask. Please add it manually.");
                  return;
              }
              throw switchError; // Throw the error to be caught by the outer catch block
          }
